...
export class AppService {
  ...
  if (!credential.client || !credential.secret || !url) {
    throw new Error('Faltan variables de entorno necesarias (Client, Secret o URL_BACKEND)');
  }
  ...
  const body = JSON.stringify({ credential, data });
  ...
  try {
    ...
    console.error('Error completo al obtener token:', error); // importante

    throw new Error(
      `Fallo al obtener token: ${status || 'Desconocido'} - ${
        data?.message || error.message || 'Sin mensaje'
      }`,
    );
  }
}
